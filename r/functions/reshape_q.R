...
reshape_q <- function(df, export_var_names = FALSE){
  ...
  for(q in unique(questions_loop[, V2])){
    ...
    if(nrow(current_q) > 0){
      ...
      current_q <- dcast(
        current_q, 
        qcountry+respondent_id+panel+wave+V4 ~ V5+V6+V7+V8, 
        value.var="value"
      )
  ...
  for(q in unique(questions_scale[, V2])){
    ...
    if(nrow(current_q) > 0){
      ...
      current_q <- dcast(current_q, qcountry+respondent_id+panel+wave+V4 ~ V5+V6+V7, value.var="value")
  ...
  if(export_var_names){
    ...
    df_codes <- merge(df_codes[respondent_id==1], 
                      ...
                      by.x="variable", 
                      by.y="V1", 
                      all.x=T)[, -c("value","respondent_id","wave", "panel")]
