...
# nboots <- c(NA, "boots_0", "boots_100", "boots_200", "boots_1000")[4]
popyear <- 2005

TEST <- FALSE

#age groups for which to create contact matrix
polymod_age_groups <- data.table(
  age_low = c(0, 5, 13, 18, seq(30, 70, 10)),
  age_high = c(4, 12, 17, seq(29, 69, 10), 100)
)
polymod_age_groups[, "name"] <- paste0(polymod_age_groups[, age_low], "-", polymod_age_groups[, age_high])
polymod_contact_age_groups <- NULL

#how to handle contact ages in age-grouo?
contact_age_process <- c("mean", "sample_uniform", "sample_popdist")[1]
...
panel_name <- paste("polymod", bootstrap_samples, sep = "_")
...
if (length(list.files(polymod_matrices_path)) == 0) {
  ...
  # contacts[, cnt_age := as.character(cnt_age_exact)]
  #contacts <- contacts[wave_id == wave_id_]
  ...
  contacts_bootstrapped <- processContacts(
    contacts, polymod_contact_age_groups, popdata, contact_age_process, contact_age_unknown_process,
  ...
  contacts_bootstrapped_matrices <- lapply(
    ...
    function(i, contacts_bootstrapped_sets, participants_bootstrapped_sets){
      ...
      lapply(
        ...
        population_data = popdata, age_groups = polymod_age_groups,
  ...
  saveRDS(
    contacts_bootstrapped_matrices,
    file.path(polymod_matrices_path, paste0("bootstrap_samples.rds"))
  )
  print(paste("saved_to", file.path(polymod_matrices_path,
                                    paste0("bootstrap_samples.rds"))))
