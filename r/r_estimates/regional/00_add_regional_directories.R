...
week_path <- paste("week", week_name, sep = "_")
survey_sub_path <- file.path(country_code_path, panel_path, week_path)
...
if (!file.exists(outputs_path)) {
  ...
  dir.create(file.path(base_outputs_path, country_code_path, panel_path, week_path))
